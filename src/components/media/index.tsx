import { Media as IMedia } from '@/payload-types'
import { ComponentProps } from 'react'
import Image from 'next/image'
import { cn } from '@/lib/utils'

type MediaProps = Pick<ComponentProps<typeof Image>, 'fill' | 'className' | 'priority' | 'style'> & {
  placeholderEnabled?: boolean
}

export const cssVariables = {
  breakpoints: {
    '3xl': 1920,
    '2xl': 1536,
    xl: 1280,
    lg: 1024,
    md: 768,
    sm: 640,
  },
}

export function Media(
  {
    media,
    className,
    fill,
    priority,
    style,
    placeholderEnabled = true,
  }: {
    media?: number | null | IMedia
  } & MediaProps) {

  if (!media || typeof media !== 'object') {
    return null
  }

  const placeholderBlur =
    'data:image/png;base64,'

  const url = media.filename ? `/media/${media.filename}` : undefined
  const { alt, width, height } = media
  if (!url) return null
  const isVideo = media?.mimeType?.includes('video')

  if (isVideo) {
    return (
      <video
        style={style}
        autoPlay
        className={cn([fill && 'absolute inset-0 size-full object-cover', className])}
        loop
        muted
        playsInline
      >
        <source src={url ?? undefined} />
      </video>
    )
  }

  const sizes = Object.entries(cssVariables.breakpoints)
    .map(([, value]) => `(max-width: ${value}px) ${value * 2}w`)
    .join(', ')
  const loading = (!priority ? 'lazy' : undefined)

  return (
    <Image placeholder={placeholderEnabled ? 'blur' : undefined}
           style={style}
           priority={priority}
           loading={loading}
           blurDataURL={placeholderEnabled ? placeholderBlur : undefined}
           fill={fill}
           className={cn([fill && 'absolute inset-0 size-full object-cover', className])}
           src={url}
           quality={100}
           sizes={sizes}
           alt={alt ?? ''}
           height={!fill ? height! : undefined}
           width={!fill ? width! : undefined} />
  )
}